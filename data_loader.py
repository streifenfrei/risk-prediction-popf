        ...
        if self.current_position < len(self._data):
            ...
            for patient_id, data_sitk, segmentation_sitk in batch_sitk:
                if self.crop == "none":
                    ...
                    data_np = np.expand_dims(np.transpose(data_np), axis=0)
                else:
                    ...
                    data_np = sitk.GetArrayFromImage(data_sitk_cropped)
                    ...
                    if self.crop in ["roi", "seg_bin", "seg"]:
                        ...
                        if self.crop == "roi":
                            mask = np.zeros_like(data_np, dtype=np.bool)
