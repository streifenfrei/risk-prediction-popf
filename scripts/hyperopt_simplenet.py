...
def objective(bb_fbeta, ir_cov, crop, drop):
    try:
        ...
        bounding_box = np.unravel_index(i, f_scores.shape) + np.array(analysis_results["bb_range"][0][0])
        ...
        _, i_min, i_max = visualize_analysis_results.crop_histogram(histogram, ir_cov)
        ...
        def get_model(input_shape):
            return models.simple_net.get_model(input_shape, dropout=drop)
        ...
        stack_trace = traceback.format_exc(e)
        return {
            "status": STATUS_FAIL,
...
def main(config):
    ...
    space = {"bounding_box_fbeta": hp.uniform("bb_fbeta", f_beta_min, f_beta_max),
             "ir_coverage": hp.uniform("ir_cov", ir_coverage_min, ir_coverage_max),
             "data_crop": hp.choice("crop", ["fixed", "roi", "seg"]),
             "dropout": hp.uniform("drop", dropout_min, dropout_max)}
    best = fmin(objective,
                space=space,
                algo=tpe.suggest,
                max_evals=config["max_evals"],
                trials_save_file=os.path.join(workspace, "save.txt"))
