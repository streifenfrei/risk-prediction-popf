...
def get_model(ct_shape=(None, None, None, 1), vector_shape=None, first_conv_channel=16, dropout=0.3):
    ct = layers.Input(shape=ct_shape)
    inputs = [ct]
    x = layers.Conv3D(filters=first_conv_channel, kernel_size=3, strides=2, activation="relu", input_shape=ct_shape)(ct)
    x = layers.BatchNormalization()(x)
